...
x_offset2 <- 1.0965*(FOV_x/2)
y_offset2 <- 1.0965*(FOV_y/2)  
...
data.frame(str_remove(str_remove(filename, "data_raw/transect_csvs_2021/"), "_v2.csv"), area, duration, alt, alt_error, ASL)
#REPLACE ASL WITH NAME OF COLUMN IN AIRDATA FILE
...
#test of single file:
#effort_catch("data_raw/manual_csvs/2020-07-14_17-33-24_v2.csv")

#calculate effort from manual files:
names_2021 <- list.files("data_raw/transect_csvs_2021", full.names = TRUE)
View(names_2021)

effort_summary_2021 <- lapply(names_2021, effort_catch)

View(effort_summary_2021)
```

```{r}
effort_summary_2021 <- effort_summary_2021[-2]

efforts_2021 <- do.call(rbind, effort_summary_2021)

transect_efforts_2021 <- dplyr::rename(efforts_2021,"datetime" = 1)
...
ggplot(manual_efforts, aes(x = area)) +
  geom_histogram()

mean_area <- transect_efforts %>% 
  #filter(area > 150000 & area < 200000) %>% 
  summarize(mean = mean(area))
mean_area
#remove outliers (for transect flights only) and calculate mean area
...
airdata <- read.csv("data_raw/transect_csvs_2021/2021-04-21_11-51-05_v2.csv")
...
```{r}
airdata <- mutate(airdata, altitude.m. = altitude.feet./3.218)
...
duration <- airdata %>% 
  filter(isTakingVideo == 1) %>% 
  mutate(altitude.m. = altitude.feet./.0348) %>% 
  select(altitude.m., time.millisecond.) %>%
  mutate(alt_grp = ifelse(altitude.m. < (alt + alt_error) & 
                          altitude.m. > (alt - alt_error), 1, 0)) %>% 
  mutate(alt_grp = rleid(alt_grp)) %>% 
