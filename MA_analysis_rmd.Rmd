...
ggplot(method_dat, aes(x = density)) +
  ...
  facet_grid(~method)
#normal distribution obviously not going to work
...
```
* Best model to explain transect sightings includes day, year, and their interaction. I just don't think that "day" is an important thing to model.
...
dredge(year_glm)
#best model to explain transect sightings includes year and hour of survey, but not area surveyed - if this were a predictive model, I would keep area in, but since it doesn't contribute additional explanatory power to the patterns that I already saw, leaving it out will give us a better understanding of the effects of year and hour:
...
```
* Best model is year + hour with no offset for area surveyed
...
### Effects testing of final "year" GLM
```{r}
Anova(year_glm_best)
#both year and hour have significant effects on sightings (which we already know from the model selection process)
...
#this is just saying that average time of survey is at 11:48 AM (and average sightings = 1.14 at that time?)
...
```
* 2019-2020 contrast isn't significant in either the simple (year-only) AOV or in the year + hour glm. This suggests that differences in the time of day at which the survey was conducted is driving increased sightings from 2019 to 2020
* Hopefully it's enough to say that  
