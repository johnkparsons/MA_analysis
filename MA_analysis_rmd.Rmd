...
gam_dat_2019_vis <- daily_3yr_dat %>%
  mutate(year = as.factor(as.character(year))) %>%
  filter(year == 2019) %>%
  mutate(hour = as.numeric(hour(date_hour))) %>%
  mutate(yday = yday(mdy)) %>% 
...
lb_gam_2019_vis <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         s(tide, k = 10) +
                         s(sft, k = 10) +
                         s(WVHT, k = 10) +
                         s(chl_interp, k = 10) +
                         vis,
                       data = gam_dat_2019_vis,
                       method = "REML", family = nb, select = TRUE)

summary(lb_gam_2019_vis)
anova(lb_gam_2019_vis)
visreg(lb_gam_2019_vis)
gam.check(lb_gam_2019_vis)

gam_dat_2019_beau <- daily_3yr_dat %>%
  mutate(year = as.factor(as.character(year))) %>%
  filter(year == 2019) %>%
  mutate(hour = as.numeric(hour(date_hour))) %>%
  mutate(yday = yday(mdy)) %>% 
  filter(transect_area_km2 > 0.17) %>% 
  mutate(vis = as.factor(beaufort))

lb_gam_2019_beau <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         s(tide, k = 10) +
                         s(sft, k = 10) +
                         s(WVHT, k = 10) +
                         s(chl_interp, k = 10) +
                         beaufort,
                       data = gam_dat_2019_beau,
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2019_beau)
...
anova(lb_gam_2019)
#DAY, WVHT, SFT, and CHL are significant in 2019
...
```{r}
summary(lb_gam_2019)
#full model DE is 89.2
#sig. terms are hour and sft
...
lb_gam_2019_sft <- mgcv::gam(total_unique ~
                                s(hour, k = 10) +
                                s(yday, k = 10) +
                                beaufort +
                                vis +
                       ...
                       data = gam_dat_2019, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2019_sft)
visreg(lb_gam_2019_sft)
#DE = 81%
90.4-81
#day DE = 9.4
...
sum(residuals(adult_gam_2019, type = "pearson")^2) / df.residual(adult_gam_2019)
...
juv_gam_2019_day <- mgcv::gam(juvenile ~  s(hour, k = 10) +
                                      beaufort +
                                      vis +
                                      s(tide, k = 10) +
                                      s(sft, k = 10) +
                       ...
                       data = gam_dat_2019, 
                       method = "REML", family = nb, select = TRUE)
summary(juv_gam_2019_day)

juv_gam_2019_day_full_surveys <- mgcv::gam(juvenile ~  s(hour, k = 10) +
                                      beaufort +
                                      vis +
                                      s(tide, k = 10) +
                                      s(sft, k = 10) +
                                      ...
                                      s(chl_interp, k = 10) +
                                      offset(area_20m),
                       data = gam_dat_2019_full_surveys_only, 
                       method = "REML", family = nb, select = TRUE)
summary(juv_gam_2019_day_full_surveys)

juv_gam_2019_full_surveys <- mgcv::gam(juvenile ~  s(hour, k = 10) +
                                      s(yday, k = 10) +
                                      beaufort +
                                      vis +
                                      s(tide, k = 10) +
                       ...
                       data = gam_dat_2019_full_surveys_only, 
                       method = "REML", family = nb, select = TRUE)
summary(juv_gam_2019_full_surveys)
anova(juv_gam_2019_full_surveys)
anova(juv_gam_2019)
```
