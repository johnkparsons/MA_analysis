...
### month model with year interaction only for hour and month (no N and S temps)
```{r}
...
### Final (as of 5/3) Global GAM structure
```{r}
gam4 <- mgcv::gam(transect_total ~ year +
                                   s(month, k = 8, by = year) +
                                   s(hour, k = 10, by = year) +
...
#month (all years) and hour (2020 and 2021) significant.
#Carp and Cabrillo temp significan, Scripps temp is not
...
### determining deviance explained of each term by dropping it from the global model - NEED TO RE-RUN ALL OF THIS with Scripps and Cabrillo temps as of 5/3
*note from Simon Wood: predictors aren't strictly orthogonal, so DE of each term might not sum to DE of global model
```{r}
summary(gam4)
#global model explains 65.9% of deviance
