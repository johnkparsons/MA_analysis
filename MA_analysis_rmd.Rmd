...
### Model selection for Cabrillo Point temp dataset
```{r}
...
### Global GAM structure (both non-local temp datasets included)
```{r}
glob_gam <- mgcv::gam(transect_total ~ year +
                                   ...
                                   s(WVHT, k = 50) +
                                   offset(transect_area_km2),
                  data = gam_dat3, method = "REML", family = nb, na.action = "na.fail")
summary(glob_gam)
