...
#Setup
```{r, eval=FALSE}
#install.packages(c("tidyverse", "dplyr", "lme4", "glmmTMB", "effects", "DHARMa", "MuMIn", plyr", "broom.mixed", "ggthemes", "padr", , "ptest", "maps", "mapdata", hexbin", "zoo", "car", "lubridate", "slider", "hms", "olsrr", "bbmle", "emmeans", "gt", "webshot", "cowplot", "jtools", "ggstance", "AER"))
...
* based on GLM with ONLY year, total unique sightings increased each year from 2019 to 2021 (p < 0.001)
...
daily_3yr_narm <- daily_3yr_dat %>% 
  ...
  filter(!is.na(transect_area_m2)) %>% 
  filter(transect_area_m2 > 10) %>% 
  mutate(transect_density = 1000000*transect_total/transect_area_m2)

season_glm <- glmmTMB(transect_total ~ year*day, family = nbinom1, data = daily_3yr_narm, na.action = "na.fail")

summary(season_glm)

dredge(season_glm)
#day*year interaction model is only good candidate 
