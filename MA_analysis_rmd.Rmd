...
#Setup
```{r, eval=FALSE}
#install.packages(c("tidyverse", "dplyr", "lme4", "glmmTMB", "effects", "DHARMa", "MuMIn", plyr", "broom.mixed", "ggthemes", "padr", , "ptest", "maps", "mapdata", hexbin", "zoo", "car", "lubridate", "slider", "hms", "olsrr", "bbmle", "emmeans", "gt", "webshot", "cowplot", "jtools", "ggstance", "AER", "mgcv", "itsadug"))
...
### Reading in LTER data
```{r}
lter_dat <- read.csv("data_raw/lter_dat.csv")
...
## DO NOT RUN: Adding wharf data - not sure why I ever needed this?
```{r}
...
#increase in median size of approximately 0.26 meters
...
```{r}
cor.test(gam_dat$tide, gam_dat$WVHT, method = "pearson")

gam_dat %>% 
  select(c(18,22,52,55,73,74)) %>% 
  cor()
#tide is correlated with month (0.44), temp (0.22), and hour (0.32), probably worth considering dropping it

plot.new()
ggplot(gam_dat, aes(x = tide, y = month)) +
  geom_point() +
  geom_smooth(method = "lm")

ggplot(gam_dat, aes(x = tide, y = Temp_top)) +
  geom_point() +
  geom_smooth(method = "lm")
#temp increases with tide

ggplot(gam_dat, aes(x = hour, y = tide)) +
  geom_point() +
  geom_smooth(method = "lm")
...
```{r}
ggplot(gam_dat, aes(x = Temp_top, y = transect_total)) + 
  geom_jitter() +
  geom_smooth() + 
  facet_grid(year~.)
...
### month model with year interaction only for hour and month
```{r}
gam4 <- mgcv::gam(transect_total ~ year +
                                   ...
                                   s(WVHT, k = 50) +
                                   offset(transect_area_km2),
                  data = gam_dat, method = "REML", family = nb, na.action = "na.fail", select = TRUE)
#select = TRUE does automated term selection to remove terms that are "infinitely smooth"

summary(gam4)
#60.7% of deviance explained - removing year interactions from envt'l terms improved DE
#hour, tide, and temp are being modeled as linear (edf close to 1)
...
#month (all years) and hour (2020 and 2021) significant smooth terms
#tide, temp, and wave height n.s.
...
# 1 is significantly lower than 3 (ratio = 0.354, p = 0.0357)
# 0 is marginally higher than 1 (ratio = 2.613, p = 0.0971)

gam4_vis_emms <- emmeans(gam4, "vis")
pwpm(gam4_vis_emms, type = "response")
#2 significantly lower than 4 (ratio = 0.462, p = 0.0279)

concurvity(gam4, full = FALSE)
```

### determining deviance explained of each term by dropping it from the global gam4 model
```{r}
summary(gam4)
#global model explains 60.7% of deviance
...
### Modelling density of juveniles
```{r}
juvgam0 <- mgcv::gam(juvenile ~ year +
                                month +
                                 s(hour, k = 10, by = year) +
                                 beaufort +
                                 vis +
                                 ...
                                 s(Temp_top, k = 10) +
                                 s(WVHT, k = 50) +
                                 offset(total_area), data = juv_dat0,
                                 method = "REML", family = nb,
                                 na.action = "na.fail", select = TRUE)
summary(juvgam0)
anova(juvgam0)
...
juvgam1 <- mgcv::gam(juvenile ~ year +
                                 ...
                                 s(Temp_top, k = 10) +
                                 s(WVHT, k = 50) +
                                 offset(total_area), data = juv_dat,
                                 method = "REML", family = nb,
                                 na.action = "na.fail", select = TRUE)

summary(juvgam1)
# 30.2% of deviance explained
...
anova(juvgam1)
#year, month in 2019 and 2020, along with temperature (!) are significant

#juvgam1_dredge_drop_1 <- dredge(juvgam1, m.min = 8, fixed = "offset(total_sized)")
juvgam1_dredge_drop_1

#juvgam1_dredge_full <- dredge(juvgam1, m.min = 3, fixed = "offset(total_sized)")
juvgam1_dredge_full
...
```{r}
summary(juvgam1)
#global model explains 30.2% of deviance
...
anova(adgam1)
#year, hour, tide, and temp significant

#adgam1_dredge_drop_1 <- dredge(adgam1, m.min = 8, fixed = "offset(total_sized)")
...
```{r}
summary(adgam1)
#global model explains 41.5% of deviance
