...
```{r}
lbgam_tot_chl_pois <- mgcv::gam(total_unique ~ year +
                                      s(hour, k = 10, by = year) +
                                      s(yday, k = 35, by = year) +
                                      beaufort +
                                      vis +
...
sum(residuals(lbgam_tot_chl_pois, type = "pearson")^2) / df.residual(lbgam_tot_chl_pois)
...
gam_dat_2019 <- daily_3yr_dat %>%
  ...
  mutate(beaufort = ifelse(beaufort == 3 | beaufort == 2, "2-3", beaufort)) %>%
  mutate(vis = ifelse(vis == 1 | vis == 2, "1-2", vis)) %>%
  mutate(vis = ifelse(vis == 4 | vis == 5, "4-5", vis)) %>%
  mutate(beaufort = as.factor(beaufort)) %>% 
  mutate(vis = as.factor(vis))
...
gam_dat_2021 <- daily_3yr_dat %>%
  ...
  filter(year == 2021) %>%
  mutate(hour = as.numeric(hour(date_hour))) %>%
  mutate(beaufort = ifelse(beaufort == 0 | beaufort == 1, "0-1", beaufort)) %>% 
  mutate(beaufort = ifelse(beaufort == 4 | beaufort == 3, "3-4", beaufort)) %>% 
  mutate(beaufort = as.factor(beaufort)) %>% 
  mutate(vis = as.factor(vis)) %>% 
  mutate(presence = ifelse(total_unique > 0, 1, 0)) %>% 
  mutate(yday = yday(mdy))
...
summary(lb_gam_2019)
#tide and wave height penalized to zero
#DE = 89.5%, n = 73
...
summary(lb_gam_2021)
#wave height and chl penalized to zero
#DE = 43.3%, n = 120

anova(lb_gam_2019)
#beaufort, hour, day, and chl are significant in 2019
...
```{r}
summary(lb_gam_2019)
#full model DE is 89.5
#sig. terms are beaufort, hour, day, and chl

lb_gam_2019_hour <- mgcv::gam(total_unique ~ 
                         s(yday, k = 10) +
                         beaufort +
                         vis +
                         s(tide, k = 10) +
                       ...
                       data = gam_dat_2019, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2019_hour)
89.5 - 81.2
#DE = 8.3

lb_gam_2019_day <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(sft, k = 10) +
                         beaufort +
                         vis +
                         s(tide, k = 10) +
                       ...
                       data = gam_dat_2019,
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2019_day)
89.5 - 83.4
#day DE = 6.1

lb_gam_2019_beaufort <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         vis +
                         s(tide, k = 10) +
                         s(sft, k = 10) +
                       ...
                       data = gam_dat_2019, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2019_beaufort)
89.5 - 82.1
#DE = 7.4

lb_gam_2019_chl <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         vis +
                         beaufort +
                         s(tide, k = 10) +
...
#DE = 48.3%
anova(lb_gam_2020)
#sig terms are day, tide, sft, waveheight

lb_gam_2020_day <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         beaufort +
                         vis +
                         s(tide, k = 10) +
                         s(sft, k = 10) +
                       ...
                       data = gam_dat_2020, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2020_day)
48.3 - 25.2
#day DE = 23.1

lb_gam_2020_tide <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         beaufort +
                         vis +
                         s(sft, k = 10) +
                       ...
                       data = gam_dat_2020, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2020_tide)
48.3-43.9
#tide DE = 4.4

lb_gam_2020_sft <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         beaufort +
                         vis +
                         s(tide, k = 10) +
                       ...
                       data = gam_dat_2020, 
                       method = "REML", family = nb, select = TRUE)
summary(lb_gam_2020_sft)
48.3-47
#sft DE = 1.3%

lb_gam_2020_wvht <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                         s(yday, k = 10) +
                         beaufort +
                         vis +
                         s(tide, k = 10) +
...
sft_2021_gam_day <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                                      beaufort +
                                      vis +
                                      s(tide, k = 10) +
                                      s(sft, k = 10) +
                       ...
                       data = gam_dat_2021, 
                       method = "REML", family = nb, select = TRUE)
summary(sft_2021_gam_day)
43.3 - 38
#day DE = 5.3

sft_2021_gam_hour <- mgcv::gam(total_unique ~ s(yday, k = 10) +
                                      beaufort +
                                      vis +
                                      s(tide, k = 10) +
                                      s(sft, k = 10) +
                       ...
                       data = gam_dat_2021, 
                       method = "REML", family = nb, select = TRUE)
summary(sft_2021_gam_hour)
43.3 - 29.7
#hour DE = 13.6

sft_2021_gam_tide <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                                      s(yday, k = 10) +
                                      beaufort +
                                      vis +
                                      s(sft, k = 10) +
                       ...
                       data = gam_dat_2021, 
                       method = "REML", family = nb, select = TRUE)
summary(sft_2021_gam_tide)
43.3 - 41.8
#tide DE = 1.5

sft_2021_gam_vis <- mgcv::gam(total_unique ~ s(hour, k = 10) +
                                      s(yday, k = 10) +
                                      beaufort +
                                      s(tide, k = 10) +
                                      s(sft, k = 10) +
...
#DE = 76.1
#day, tide, wave height, and chl penalized to zero
anova(juv_gam_2019)
...
#DE = 27.9%
#hour, day, tide, and sft penalized to zero
...
```{r}
# full DE = 26.2%, day and temp are significant
...
summary.gam(juv_gam_2020)

ggplot(gam_dat_2020, aes(x = sft, y = juvenile)) +
  geom_point() +
  geom_smooth()
```
...
```{r}
#DE = 27.9, only sig. variable is wvht
...
27.9 - 25.1
#DE = 2.8
#somewhat sensitive in that day becomes significant
```
...
### Dropping significant terms from 2021 juvenile model
```{r}
#DE = 13, hour significant
...
```{r}
# DE = 21.7%, sig. terms are day, tide and sft
