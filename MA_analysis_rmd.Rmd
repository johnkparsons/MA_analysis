...
### Looking at trends with the detectability variables
```{r}
ggplot(gam_dat, aes(x = beaufort, y = transect_total)) + 
  geom_jitter()

ggplot(gam_dat, aes(x = vis, y = transect_total)) + 
  geom_jitter()
```

### temporal and detectability variables, no interactions
```{r}
gam1 <- mgcv::gam(transect_total ~ year + s(hour, bs = "cc", k = 10) + s(day, bs = "cc", k = 100) + beaufort + vis + offset(transect_area_km2), data = gam_dat, method = "REML", family = nb)
#REML used for small dataset, to prevent over-smoothing
...
### temporal and detectability variables, with year interactions for day and hour
```{r}
gam <- mgcv::gam(transect_total ~ year + s(hour, bs = "cc", by = year) + s(day, bs = "cc", by = year) + beaufort + vis + offset(transect_area_km2), data = gam_dat, method = "REML", family = nb)
