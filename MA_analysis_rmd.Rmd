...
### Building 3yr daily dataset WITH BUOY DATA - SEE PREVIOUS CHUNK
```{r}
str(dat_2019_full)
...
yearly_abund_summary <- daily_3yr_dat %>% 
  ...
  summarize("Total sightings" = sum(total_unique, na.rm = TRUE),
            "Mean abundance" = mean(total_unique, na.rm = TRUE),
  ...
  gt() %>% 
    ...
    cols_width("Total sightings" ~ px(80),
               "Mean abundance" ~ px(80),
               "Maximum abundance" ~ px(80),
               "Sightings per day" ~ px(80)) %>% 
    cols_align(align = "center")
gtsave(yearly_abund_summary, "plots/yearly_abund_summary.png")

#years overlaid (not very informative):
daily_3yr_dat %>% 
  group_by(year(date), week(date)) %>% 
  summarize(mean = mean(total_unique, na.rm = TRUE)) %>%
  rename(week = "week(date)", year = "year(date)") %>% 
  ungroup() %>% 
  ggplot(aes(x = as_factor(week), y = mean, fill = as_factor(year))) +
    geom_bar(stat = "identity")

#years faceted (decent option, potentially as a second graph after years averaged?):
daily_3yr_dat %>% 
  group_by(year(date), week(date)) %>% 
  summarize(mean = mean(total_unique, na.rm = TRUE)) %>%
  rename(week = "week(date)", year = "year(date)") %>% 
  ungroup() %>% 
  ggplot(aes(x = as_factor(week), y = mean)) +
    geom_bar(stat = "identity") +
    facet_grid(.~as_factor(year))

#years averaged: NEED TO FIX X-AXIS
daily_3yr_dat %>% 
  group_by(week(date)) %>% 
  summarize(mean = mean(total_unique, na.rm = TRUE)) %>%
...
#size summary table in METERS, adults per juvenile
yearly_size_summary <- size_dat_unique %>% 
  ...
  gt() %>%
    ...
    cols_width("Adults per Juvenile" ~ px(100)) %>% 
    cols_align(align = "center")
