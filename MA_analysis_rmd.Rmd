...
#2021 has a LOT more days where there were only adult-sized sharks, and 2020 has tons of days where only juvenile-sized sharks were seen
```
...
summary(gam4)
#60.7% of deviance explained - removing year interactions from envt'l terms improved fit
...
### day model, year interactions with hour and day (like gam4 but with day, this is probably the best option)
```{r}
...
propgam1 <- mgcv::gam(juvenile ~ year +
                                   ...
                                   s(Temp_top, k = 10) +
                                   s(WVHT, k = 50) +
                                   offset(total_sized), data = gam_dat, method = "REML", family = nb, na.action = "na.fail")

summary(propgam1)
# only 10.3% of deviance explained. basically everything is being modelled as a flat line
