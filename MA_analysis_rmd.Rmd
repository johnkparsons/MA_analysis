...
daily_3yr_dat %>% 
  group_by(year) %>% 
  summarize(surveys = n(), mean_density = mean(total_unique/total_area, na.rm = TRUE), sd_density = sd(total_unique/total_area, na.rm = TRUE), median_density = median(total_unique/total_area, na.rm = TRUE), range_density = max(total_unique/total_area, na.rm = TRUE) - min(total_unique/total_area, na.rm = TRUE))
...
ggplot(aes(x = as_date(mdy), y = sft), data = daily_3yr_dat) +
  geom_point() +
...
mgcv::concurvity(lb_gam_2021, full = FALSE)
```
...
visreg(lbgam_all, scale="response", cond=list(total_area=1))
...
gam.check(lbgam_juv)
#issue with sft, will look at residuals and sensitivity
visreg(lbgam_juv)
...
visreg(lbgam_adult, scale = "response", cond = list(area_20m = 1))
...
lbgam_all_sst_no_sst <- mgcv::gam(total_unique ~ year +
                                      s(hour, k = 10) +
                                      s(yday, k = 10, by = year) +
                                      beaufort +
                                      vis +
