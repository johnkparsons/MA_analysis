...
### Visualizing manual vs transect
```{r}
...
ggplot(comp_vis_dat, aes(x = metric, y = value)) +
  geom_boxplot(aes(color = method)) +
  geom_point(aes(x = metric, y = 139), shape = 8) #in progress...
...
### Final modeling procedure - Full survey window
```{r}
...
year_glm_best <- glmmTMB(transect_total ~ year + day + hour, family = nbinom1, data = daily_3yr_narm, na.action = "na.fail")
...
### Effects testing of final "year" GLM - Full survey window
```{r}
Anova(year_glm_best)
...
# 2021 mean = 2.82171
```
* 2019-2020 contrast isn't significant in either the simple (year-only) AOV or in the year + day + hour glm. This suggests that other factors than the ones examined here are driving increased sightings from 2019 to 2020
