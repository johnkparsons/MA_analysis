...
#Setup
```{r, eval=FALSE}
#install.packages("tidyverse", "dplyr", "lme4", "glmmTMB", "effects", "DHARMa", "plyr", "broom.mixed", "ggthemes", "padr", "hexbin", "zoo", "car", "lubridate", "slider", "hms", "olsrr", "bbmle", "emmeans", "gt", "webshot")
...
### Reading in environmental data - 2019
```{r}
channel_dat_2019_raw <- read.csv("data_raw/2019_channel_data.csv")
...
dat_2019_full <- flights_plus_channel_dat_2019 %>%
...
* may have to pull in more code from first chunk once I start adding in environmental data
```{r}
...
### Reading in environmental data - 2020
```{r}
channel_dat_2020_raw <- read.csv("data_raw/2020_channel_data.csv")
str(channel_dat_2020_raw)

channel_dat_2020 <- channel_dat_2020_raw %>%
  mutate_if(is.character, as.numeric) %>% 
  mutate_at(vars(WVHT_m, DPD_sec, APD_sec), ~na_if(., 99)) %>%  
  mutate_at(vars(MWD_deg, ATMP, WTMP, DEWP), ~na_if(., 999)) %>%
  group_by(MM, DD, hh) %>% 
  summarize_at(vars(3:13), mean, na.rm = TRUE) %>%
  ungroup() %>% 
  filter(MM > 4) %>% 
...
dat_2020_full <- flights_plus_channel_dat_2020 %>%
  mutate(wtemp_2 = mean_wtemp_previous_24^2)
```
...
#View(size_dat_2020)
size_dat_2020 <- size_dat_2020 %>% slice(4:nrow(size_dat_2020))
...
### Reading in environmental data - 2021
```{r}
channel_dat_2021_raw <- read.csv("data_raw/2021_channel_data.csv")
str(channel_dat_2021_raw)

channel_dat_2021 <- channel_dat_2021_raw %>%
                      mutate_if(is.character, as.numeric) %>% 
                      mutate_at(vars(WVHT, DPD, APD, MWD), ~na_if(., 99)) %>%  
                      mutate_at(vars(ATMP, WTMP, DEWP), ~na_if(., 999)) %>%
                      group_by(MM, DD, hh) %>% 
                      summarize_at(vars(3:13), mean, na.rm = TRUE) %>%
                      ungroup() %>% 
                      filter(MM > 3) %>% 
...
dat_2021_full <- flights_plus_channel_dat_2021 %>%
  mutate(wtemp_2 = mean_wtemp_previous_24^2)
```
...
#View(size_dat_2021)
size_dat_2021 <- size_dat_2021 %>% slice(3:nrow(size_dat_2021))
...
size_dat_2021 %>% 
  filter(size_class == "Juvenile") %>% 
  nrow()
#MUCH higher % of adults than 2020 - 3:4 ratio
```

## Three-Year ("3yr") dataset tidying

### Building 3yr daily dataset
```{r}
str(dat_2019_full)
...
#daily_3yr_dat <- select(daily_3yr_dat, -c(7:8,23:25, 86:87))
...
write.csv(size_dat_3yr, "size_dat_3yr.csv")
```
...
```{r}
size_dat_3yr <- read.csv("size_dat_3yr.csv")
...
```{r}
daily_3yr_dat <- full_join(dat_2019, dat_2020)
#this isn't the "real" 3 yr dataset I'll use - see above
yearly_abund_summary <- daily_3yr_dat %>% 
  group_by(year) %>% 
  summarize(mean = mean(total_unique, na.rm = TRUE),
            max = max(total_unique, na.rm = TRUE))
yearly_abund_summary
...
size_dat_unique %>% 
  ...
  ggplot(aes(x = as.factor(year), y = length)) +
    geom_boxplot() #+
    #geom_hline(yintercept = 3)
ggsave("plots/size_3yrs_box.png")
