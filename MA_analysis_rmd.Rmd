...
size_dat_2019 %>%
  ...
  nrow()
```
*no adults with unadjusted lengths, 14 with adjusted lengths!
...
#ggplot(sft_19, aes(group = hour(date_hour), y = sft)) +
  #geom_boxplot()
...
station_filtered_20_21 <- filtered_temp_20_21_raw %>% 
  filter(Station == "JWS_SantaClaus_OS1" | Station == "JWS_SantaClaus_OS2" | 
         ...
         Station == "JWS_Padaro_Inshore_C" | Station == "JWS_Padaro_Inshore_D" | Station == "JWS_Padaro_Buoy") %>% 
...
#station_filtered_20_21 %>% 
  #group_by(year(DateTimeUTC), Station, depth_level) %>% 
  #summarize(rows = n()) %>% 
  #View()
...
sst_20_21 <- station_filtered_20_21 %>% 
  ...
  mutate(date_hour = make_datetime(year(date), month(date),
                                   day(date), hour)) %>% 
  select(!c(1,2)) %>% 
  mutate(date_hour = with_tz(date_hour, tzone = "America/Los_Angeles"))
...
ggplot(sft_sst_20_21, aes(x = hour(date_hour), y = sst)) +
  geom_point()
#time-zone shift worked correctly 
...
transect_vis_dat <- read.csv("data_raw/transect_vis.csv")
...
size_dat_3yr %>%
  ...
  summarize(sharks = n())
99 + 168
#267 observations clear enough to determine tagged/untagged status
#of these, 99 were not tagged and 168 were
99/267
...
size_dat_3yr %>% 
  ...
  summarize(median = median(length_adj_m, na.rm = TRUE))
...
daily_3yr_dat %>% 
  filter(Temp_top < 100) %>% 
  ggplot(aes(x = as_datetime(date_hour))) +
...
anova(lbgam_all)
#year, day, and hour all significant (p < 0.001)
#vis significant (p < 0.001), beaufort not
#tide (p < 0.05) and sft (p < 0.005) significant, waveheight n.s., chl marginal (p = 0.069)

visreg(lbgam_all, "year", cond=list(total_area=1))
...
anova(lbgam_juv)
#year and day significant (p < 0.05)
#hour marginal (p = 0.59)
#vis significant (p < 0.05), beaufort not
#sft significant (p < 0.001), tide, waveheight and chl not
...
visreg(lbgam_adult, "sft")

anova(lbgam_adult)
#year significant, day significant in 2021 only
#hour marginal
#vis significant, beaufort not
#sft sig. (p < 0.05) waveheight, chl and tide not 
...
```{r}
sft_all <- visreg(lbgam_all, "sft")
View(sft_all)

# for response scale, add: , scale = "response", cond = list(area_20m = 1)
...
sft_adult <- visreg(lbgam_adult, "sft")
...
sft_juv$fit$Model <- "Small"
sft_adult$fit$Model <- "Large"
...
sft_bind$Model <- factor(sft_bind$Model, levels = c("Overall", "Small", "Large"))
...
ggplot(sft_bind, aes(x = sft, y = visregFit)) +
  ...
  theme_cowplot() +
  scale_x_continuous(breaks = c(10,12,14,16,18,20,22,24)) +
  labs(x = bquote("Sea Floor Temperature (\u00B0C)"), y = "Relative Density") +
  theme(panel.background = element_rect(fill = "white", colour = NA),
      plot.background = element_rect(fill = "white", colour = NA)) 
...
yday_dat <- visreg(lbgam_all, "yday", by = "year",scale = "response", cond = list(total_area = 1))
View(yday_dat)
...
visreg(lbgam_all_sst, "sst")
visreg(lbgam_all, "sft")

```

### SST SMALL SHARKS FOR THESIS
```{r}
lbgam_small_sst <- mgcv::gam(juvenile ~ year +
                                      s(hour, k = 10) +
                                      s(yday, k = 10, by = year) +
                                      beaufort +
                                      vis +
                       ...
                       data = gam_dat_lb, method = "REML",
                       family = nb, select = TRUE)
summary(lbgam_small_sst)
anova(lbgam_small_sst)
