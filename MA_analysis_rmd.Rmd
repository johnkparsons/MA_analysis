...
### Do detections vary across the field season? (NEED TO REPLICATE THIS WITH TOTAL DENSITY (total_unique/total_area) INSTEAD OF TOTAL UNIQUE COUNT)
```{r}
daily_3yr_dat <- mutate(daily_3yr_dat, day = yday(date))
daily_3yr_dat <- mutate(daily_3yr_dat, day2 = (yday(date))^2)

season_lm <- lm(total_unique ~ day, data = daily_3yr_dat)
summary(season_lm)
#no apparent linear trend

season2_lm <- lm(total_unique ~ day + day2, data = daily_3yr_dat)
summary(season2_lm)
...
season_glm <- glmmTMB(total_unique ~ year*day, family = nbinom1, data = daily_3yr_narm, na.action = "na.fail")
summary(season_glm)
...
```
* Best model to explain total unique sightings includes day, year, and their interaction.
