...
size_dat_2020 %>% 
  filter(length_adj_m >= 3) %>% 
  nrow()
#34 adults unadjusted
#70 adults adjusted

size_dat_2020 %>% 
  filter(length_adj_m < 3) %>% 
  nrow()
#324 juveniles unadjusted
#288 adults adjusted
```
* roughly 10x more juveniles than adults (uncorrected lengths), goes down to 4:1 juvenile:adult when adjusted lengths used
...
```{r}
daily_3yr_dat <- left_join(daily_3yr_dat, lter_dat, by = "date_hour")
...
size_dat_3yr %>% 
  ...
  count(class_adj)
#125 adults and 475 juveniles with raw measurements, 219 adults and 381 juveniles with adjusted measurements - adjustments bring 49 sharks from juvenile to adult length
...
ggplot(data = daily_3yr_dat, aes(x = as.factor(year(date_hour)), y = juvenile/total_sized)) +
  geom_boxplot()
...
### transect total GLM, no interactions
```{r}
gam1 <- mgcv::gam(transect_total ~ year + 
                                   s(hour, bs = "cc", k = 10) +
                                   s(day, bs = "cc", k = 20) +
                                   beaufort + vis + s(tide, bs = "cr", k = 10) +
...
### transect total GLM, with year interactions
```{r}
...
#knots hand-selected to be as large as possible without throwing "more coefficients than data" error

summary(gam2)
#adding year interactions increases deviance explained from 37 to 64 percent
gam.check(gam2)
...
### comparing interaction and non-interaction models:
```{r}
AIC(gam1, gam2)
```
...
summary(gam3)
#month model explains 60% of deviance compared to 64% for day model
...
### month model with year interaction only for hour and month (as of 4/12 I think this is what I should use)
```{r}
...
anova.gam(gam4)
#year, beaufort, vis, hour, month all significant

visreg(gam4)

dredge(gam4, m.min = 8)
#dropping area, tide, vis, waveheight, and temp all (marginally) improves fit over global model
#dropping beaufort, hour, month, and year all significantly worsen fit compared to global model

gam4_full_dredge <- dredge(gam4, m.min = 4)
```
