...
ggplot(gam_dat, aes(x = Temp_top, y = juvenile/total_sized)) + 
  geom_jitter() +
  geom_smooth() + 
  facet_grid(year~.)
...
prop_aov <- aov((juvenile/total_sized) ~ year, data = prop_dat)
summary(prop_aov)
plot(prop_aov)

TukeyHSD(prop_aov)
#2021 significantly higher than 2020
```
...
summary(propgam1)
# only 7.15 % of deviance explained. basically everything is being modelled as a flat line
...
#vis hurts the model the most
#worst term to remove is month, followed by wave height and then tide
