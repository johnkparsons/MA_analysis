...
#when no sharks are detected in transect, manual detects 0.539
```
...
* year/hour interaction improves fit
* adding area surveyed increases AIC (area has very low variation, so it makes sense that adding it as a predictor doesn't add much info)
* 2019-2020 contrast isn't significant in either the simple (year-only) AOV or in the year*hour glm, but it is significant in the year-only glm. This suggests that differences in survey time is driving increased sightings from 2019 to 2020!
...
year_gam <- mgcv::gam(transect_total ~ year + s(hour, by = year), data = gam_dat)
