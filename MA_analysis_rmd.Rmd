...
lter_dat %>% 
  ...
  ggplot(aes(x = date_hour, y = Temp_top)) +
    geom_point() +
    geom_smooth()
```
...
### Recovery of clean daily_3yr_dat (so you don't have to re-run everything if you mess it up)
```{r}
...
gam4 <- mgcv::gam(transect_total ~ year +
                  ...
                  family = nb, select = TRUE, na.action = "na.fail")
...
gam4lb <- mgcv::gam(transect_total ~ year +
                                   ...
                                   s(WVHT, k = 50) +
                                   offset(transect_area_km2),
                  data = gam_dat_lb, method = "REML", family = nb, select = TRUE, na.action = "na.fail")
#select = TRUE does automated term penalization to remove terms that are "infinitely smooth"
...
gam4lb_dredge_drop1 <- dredge(gam4lb, m.min = 8, fixed = "offset(transect_area_km2)")
gam4lb_dredge_drop1
# global model is slightly better than dropping tide or waveheight

#gam4lb_full_dredge <- dredge(gam4lb, m.min = 3, fixed = "offset(transect_area_km2)")
gam4lb_full_dredge
...
### Comparing total and transect count w/CSULB data
```{r}
...
### determining deviance explained of each term by dropping it from the global model CSULB VERSION - as of 6/16 
