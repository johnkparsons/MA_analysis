...
View(padaro_sst_19_raw)
#this is SST from one buoy from 2019-2020
...
padaro_sst_20_21 <- filtered_temp_20_21 %>% 
  ...
  filter(LoggerDepth_m == 1) %>%
  rename(sst = 6) %>% 
  rename(datetime_UTC = 1)
View(padaro_sst_20_21)

padaro_sst_raw <- bind_rows(padaro_sst_19, padaro_sst_20_21)
View(padaro_sst_raw)
write.csv(padaro_sst_raw, "data/padaro_sst_raw.csv")
...
daily_3yr_dat_temp <- left_join(daily_3yr_dat, padaro_sst, by = "date_hour")

ggplot(daily_3yr_dat, aes(x = date_hour)) +
  geom_point(aes(y = sst)) +
  geom_point(aes(y = Temp_top), color = "red") +
  ylim(c(0,26))
...
daily_3yr_dat %>% 
  ggplot(aes(x = Temp_top, y = sst)) +
    geom_point() +
    geom_smooth(method = "lm")

lter_vs_csulb_lm <- lm(Temp_top ~ sst, data = daily_3yr_dat)
summary(lter_vs_csulb_lm)

cor.test(daily_3yr_dat$Temp_top, daily_3yr_dat$sst, method = "pearson")
#0.66 correlation coefficient btwn CSULB and LTER data

padaro_SFT_raw <- read.csv("data_raw/CSULB/CSULB Shark Buoys_SFT.csv")
View(padaro_SFT_raw)
str(padaro_SFT_raw)

#filter(Sensor == "aquaMeasure-670473") %>%
padaro_SFT_DO <- padaro_SFT_raw %>%
  rename(datetime_UTC = 1) %>% 
  rename(sft = 8) %>%
  rename(DO = 13) %>% 
  mutate(datetime_UTC = as_datetime(datetime_UTC)) %>% 
  mutate(datetime_PST = force_tz(datetime_UTC, tzone = "America/Los_Angeles")) %>%
  group_by(date(datetime_PST), hour(datetime_PST)) %>% 
  summarize(sft = mean(sft, na.rm = TRUE), DO = mean(DO, na.rm = TRUE)) %>% 
  rename(date = 1, hour = 2) %>% 
  mutate(date_hour = make_datetime(year(date), month(date), day(date), hour))

daily_3yr_dat_temp <- left_join(daily_3yr_dat_temp, padaro_SFT_DO, by = "date_hour")
View(daily_3yr_dat_temp)

ggplot(daily_3yr_dat_temp, aes(x = date)) +
  geom_point(aes(y = sst))
```
...
```{r}
#2019 SST:
sst_2019_raw <- read.csv("data_raw/CSULB/2019_Live_buoy_SST.csv")
View(sst_2019_raw)

#2019 SFT:
sft_2019_raw <- read.csv("data_raw/CSULB/2019_Live_buoy_SFT.csv")
View(sft_2019_raw)
