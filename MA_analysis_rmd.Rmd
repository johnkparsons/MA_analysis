...
#dat_2020 <- dat_2020 %>% 
                  #rename(WDIR = WDIR_deg,
                        ...
                        # MWD = MWD_deg,
                         #PRES = PRES_hPa)
dat_2020$datetime.x <- as.character(dat_2020$datetime.x)  
#dat_2020 <- select(dat_2020, -(17:19))
...
daily_2yr_dat$datetime.x <- as_datetime(daily_2yr_dat$datetime.x)
...
daily_3yr_dat <- mutate(daily_3yr_dat, year = as_factor(year(date)))
...
```{r}
dat_no_19$date_hour <- as_datetime(dat_no_19$date_hour)
dat_no_19 <- mutate(dat_no_19, year = year(date_hour))
#View(dat_no_19)
...
```
* negative number on histogram means more sharks seen in manual - to me this is good evidence that we should use manual count: it is simply seeing more sharks
...
method_glm <- glmmTMB(count ~ method + offset(log(area)), data = method_glm_dat)
summary(method_glm)

met_res <- simulateResiduals(method_glm)
plot(met_res)
```
...
```{r}
daily_3yr_dat <- mutate(daily_3yr_dat, day = yday(date))
daily_3yr_dat <- mutate(daily_3yr_dat, day2 = (yday(date))^2)
...
dredge(season_glm)
#day*year interaction model is best candidate - but why do I even care about day? 
...
```
* Best model to explain transect sightings includes day, year, and their interaction. I just don't think that "day" isan important thing to model.

### Alternative approach to comparing years (no "day", just looking at survey-related variables like time of day, area surveyed, etc.)
```{r}
daily_3yr_narm <- daily_3yr_narm %>% 
...
dredge(year_glm)
#year*hour w/out area is best:

year_glm_best <- glmmTMB(transect_total ~ year*hour, family = nbinom1, data = daily_3yr_narm, na.action = "na.fail")

summary(year_glm_best)

yr_res <- simulateResiduals(year_glm_best)
plot(yr_res)
#no issues
pairs(emmeans(year_glm_best, ~year))
#not a good test

year_only_glm <- glmmTMB(transect_total ~ year, family = nbinom1, data = daily_3yr_narm, na.action = "na.fail")
pairs(emmeans(year_only_glm, ~year))
...
```
* hour and year are important predictors - all 4 models with any weight include both - but best model by far is year*hour with no offset for area surveyed
* hour-only model is actually slightly better than year-only model, which is  interesting
* adding area surveyed increases AIC (area has very low variation, so it makes sense that adding it as a predictor doesn't contribute much info)
* 2019-2020 contrast isn't significant in either the simple (year-only) AOV or in the year*hour glm, but it is significant in the year-only glm. This suggests that differences in the time of day at which teh survey was conducted is driving increased sightings from 2019 to 2020!

### Copying above approach but with GAM
```{r}
...
year_gam <- mgcv::gam(transect_total ~ year + s(hour) + offset(transect_area_km2), data = gam_dat)
...
### Year is categorical so it doesn't make sense to use GAM unless hour has a non-linear effect on count:
```{r}
ggplot(gam_dat, aes(x = hour, y = transect_total))+
  geom_jitter() +
  geom_smooth()
```
