...
#size_classed_3yr_20m_test <- joined_3yr %>%
  ...
  #dplyr::summarize(juvenile = sum(length_adj_m < 3), 
                   ...
                   #juvenile_20m = sum(length_adj_m < 3 &
                                        #ifelse(year == "2019",
                                               #length_adj_m > 0,
                                               #(video_YN == "manual" | 
                                                  #video_YN == "manual_1"))),
...
daily_3yr_dat <- daily_3yr_dat  %>%
  ...
  mutate(total_area = ifelse(year == 2019, transect_area_km2, ifelse(is.na(manual_area_km2), transect_area_km2, ifelse(is.na(transect_area_km2), manual_area_km2, manual_area_km2 + transect_area_km2)))) %>% 
...
scripps_dat <- scripps_dat_raw %>% 
               ...
               group_by(mdy) %>% 
               summarize(scripps_temp = mean(temp, na.rm = TRUE)) %>% 
...
cabrillo_dat <- cabrillo_dat_raw %>% 
               ...
               mutate(mdy = as_date(time)) %>% 
               group_by(mdy) %>% 
               summarize(cabrillo_temp = mean(temp, na.rm = TRUE)) %>% 
...
### Working version as of 6/28: day instead of month, CSULB or LTER sft data, total count
```{r}
######################
...
#year and vis significant parametrics, beaufort marginal at p = 0.068
...
### Dropping terms to test sensitivity to concurvity
```{r}
...
sst_2021_gam <- mgcv::gam(total_unique ~ year +
                                      s(hour, k = 10, by = year) +
                                      s(month, k = 8, by = year) +
                                      beaufort +
                                      vis +
...
sft_2021_gam <- mgcv::gam(total_unique ~ year +
                                      s(hour, k = 10, by = year) +
                                      s(month, k = 8, by = year) +
                                      beaufort +
                                      vis +
